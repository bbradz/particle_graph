...
If (i_par.Eq.1) Then 
If (i_c.Eq.0) mftINPUT= Cmplx(wert,Aimag(mftINPUT),dp) 
If (i_c.Eq.1) mftINPUT= Cmplx(Real(mftINPUT,dp),wert,dp) 
Else If (i_par.Eq.2) Then 
If (i_c.Eq.0) LamINPUT= Cmplx(wert,Aimag(LamINPUT),dp) 
If (i_c.Eq.1) LamINPUT= Cmplx(Real(LamINPUT,dp),wert,dp) 
Else
...
l_open= .False.
End If
Write(io_L,100) "# SUSY Les Houches Accord 2 - NM Spectrum + Decays + Flavour Observables"
...
Write(io_L,101) 1, Real(mftINPUT,dp) ,"# mftINPUT"
Write(io_L,101) 2, Real(LamINPUT,dp) ,"# LamINPUT"
WriteNextBlock = .False. 
If (Abs(Aimag(mftINPUT)).gt.0._dp) WriteNextBlock = .True. 
If (Abs(Aimag(LamINPUT)).gt.0._dp) WriteNextBlock = .True. 
If(WriteNextBlock) Then 
Write(io_L,100) "Block IMMINPAR  # Input parameters"
If (Abs(Aimag(mftINPUT)).gt.0._dp) Then 
Write(io_L,101) 1, Aimag(mftINPUT) ,"# mftINPUT"
End if 
If (Abs(Aimag(LamINPUT)).gt.0._dp) Then 
Write(io_L,101) 2, Aimag(LamINPUT) ,"# LamINPUT"
End if 
End if 
...
Subroutine WriteWHIZARD 
   Open(123,file="WHIZARD.par.NM",status="unknown")
