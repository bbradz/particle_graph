...
If (i_par.Eq.1) Then 
If (i_c.Eq.0) MtINPUT= Cmplx(wert,Aimag(MtINPUT),dp) 
If (i_c.Eq.1) MtINPUT= Cmplx(Real(MtINPUT,dp),wert,dp) 
Else If (i_par.Eq.2) Then 
If (i_c.Eq.0) LambdaVari4INPUT= Cmplx(wert,Aimag(LambdaVari4INPUT),dp) 
If (i_c.Eq.1) LambdaVari4INPUT= Cmplx(Real(LambdaVari4INPUT,dp),wert,dp) 
Else
...
Complex(dp) :: PDGAh,PDGhh,PDGHp,PDGVP,PDGVZ,PDGVWp,PDGVG,PDGgP,PDGgZ,PDGgWp,PDGgWpC,PDGgG,          & 
& PDGFv(3),PDGFe(3),PDGFu(3),PDGFd(3)
...
l_open= .False.
End If
Write(io_L,100) "# SUSY Les Houches Accord 2 - TNMA Spectrum + Decays + Flavour Observables"
...
Write(io_L,101) 1, Real(MtINPUT,dp) ,"# MtINPUT"
Write(io_L,101) 2, Real(LambdaVari4INPUT,dp) ,"# LambdaVari4INPUT"
WriteNextBlock = .False. 
If (Abs(Aimag(MtINPUT)).gt.0._dp) WriteNextBlock = .True. 
If (Abs(Aimag(LambdaVari4INPUT)).gt.0._dp) WriteNextBlock = .True. 
If(WriteNextBlock) Then 
Write(io_L,100) "Block IMMINPAR  # Input parameters"
If (Abs(Aimag(MtINPUT)).gt.0._dp) Then 
Write(io_L,101) 1, Aimag(MtINPUT) ,"# MtINPUT"
End if 
If (Abs(Aimag(LambdaVari4INPUT)).gt.0._dp) Then 
Write(io_L,101) 2, Aimag(LambdaVari4INPUT) ,"# LambdaVari4INPUT"
End if 
End if 
...
Write(io_L,104) 1,Real(LambdaVari4,dp), "# LambdaVari4" 
WriteNextBlock = .false. 
If (OutputForMG) WriteNextBlock = .True. 
...
WriteNextBlock = .false. 
If (OutputForMG) WriteNextBlock = .True. 
Write(io_L,106) "Block TREEMixedBlock Q=",Q,"# (Renormalization Scale)" 
Write(io_L,104) 1,Real(mu2Tree,dp), "# mu2" 
If (Abs(Aimag(mu2Tree)).gt.0._dp) WriteNextBlock = .True. 
If(WriteNextBlock) Then 
Write(io_L,106) "Block TREEIMMixedBlock Q=",Q,"# (Renormalization Scale)" 
Write(io_L,104) 1,Aimag(mu2Tree), "# mu2" 
End if 
WriteNextBlock = .false. 
If (OutputForMG) WriteNextBlock = .True. 
Write(io_L,106) "Block LOOPMixedBlock Q=",Q,"# (Renormalization Scale)" 
Write(io_L,104) 1,Real(mu21L,dp), "# mu2" 
If (Abs(Aimag(mu21L)).gt.0._dp) WriteNextBlock = .True. 
If(WriteNextBlock) Then 
Write(io_L,106) "Block LOOPIMMixedBlock Q=",Q,"# (Renormalization Scale)" 
...
Write(io_L,104) 1,Real(LambdaVari4GUT,dp), "# LambdaVari4" 
...
Write(io_L,1101) rHB_S_S_Fu(1,3),rHB_S_P_Fu(1,3), 3 ,25,9269346,9269346, " # h_1 t t coupling  " 
...
If ((0..gt.MinWidth).or.(OutputForMG)) Then 
...
Subroutine WriteWHIZARD 
   Open(123,file="WHIZARD.par.TNMA",status="unknown")
