...
class FermionField(Field):
    ...
    def __check__(self):
        ...
        self.checklist["ptcl_check"] = ptcl_check
    ...
    def _full_reps(self):
        for key, group in self.groups.items():
            ...
            if group.abelian:
                ...
                self.indices.append(self._index(group, rep_dict["dim"])) if self._index(group, rep_dict["dim"]) is not None else None
    ...
    def _unphy_field_info(self):
        unphy_field = {
            "ClassName": self.name,
            "Unphysical": True,
            "Indices": self.rewrite_Indices(self.indices),
            "FlavorIndex": self.gen_idx,
            "SelfConjugate": self.self_conjugate,
            "QuantumNumber": self.rewrite_QuantumNumber(self.QuantumNumber),
            "Definition": ""    
        }
        return unphy_field
