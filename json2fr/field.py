...
class Field:
    ...
    def __check__(self):
        ...
        try:
            # check if the particles are massive
    ...
    def __validate__(self):
        all_checks = [self._generation_index, self._gen_type_consistency, 
                      self._full_reps, self._dim_consistency]
...
class FermionField(Field):
    ...
    def __init__(self, id, name, reps, groups, dim, gen, particles, self_conjugate, QuantumNumber, chirality):
        ...
        super().__init__(id, name, "fermion", reps, groups, dim, gen, particles, self_conjugate, QuantumNumber)
        self.__check__()
        self.color = 1
        self.mass_type = None
    ...
    # Check if all the particles pass all the checks
    ...
    def __validate__(self):
        super().__validate__()
        all_checks = [self._assign_colors, self._sort_unphy_fields, 
    ...
    def _phy_field_info(self):
        assert self.pass_all_checks(), f"AssertionError: {self.name} has failed the checks"
        phy_fields = []
        for i in range(self.dim):
    ...
    def _check_mass_type(self):
        if self.is_massive:
            assert self.mass_type is not None, f"AssertionError: {self.name} is massive but has no mass type"
        else:
            pass
...
class ScalarField(Field):
    def __init__(self, id, name, type, reps, groups, dim, gen, particles, self_conjugate, QuantumNumber):
        self.subtype = type
        super().__init__(id, name, "scalar", reps, groups, dim, gen, particles, self_conjugate, QuantumNumber)
        self.__check__()
        self.potential = None
        self.vev = None
        self.get_vev = False
    ...
    def __check__(self):
        ...
        self.checklist.update({"subtype_check": subtype_check})
...
class VectorField(Field):
    def __init__(self, id, name, reps, groups, dim, gen, particles, self_conjugate, QuantumNumber):
        super().__init__(id, name, "vector", reps, groups, dim, gen, particles, self_conjugate, QuantumNumber)
        self.__check__()
